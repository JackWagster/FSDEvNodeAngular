...
export class BookStoreComponent implements OnInit {
  ...
  constructor(protected bookStoreService: BookStoreService, protected modalService: NgbModal, protected parseLinks: ParseLinks) {
    this.bookStores = [];
    ...
    this.bookStoreService
      ...
      .subscribe(
        (res: HttpResponse<IBookStore[]>) => {
          ...
          this.paginateBookStores(res.body, res.headers);
        },
        () => {
          this.isLoading = false;
        }
  ...
  protected paginateBookStores(data: IBookStore[] | null, headers: HttpHeaders): void {
    this.links = this.parseLinks.parse(headers.get('link') ?? '');
    if (data) {
      for (const d of data) {
        this.bookStores.push(d);
      }
    }
  }
}
