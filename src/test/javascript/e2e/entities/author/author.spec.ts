...
describe('Author e2e test', () => {
  ...
  it('should create and save Authors', async () => {
    ...
    await promise.all([authorUpdatePage.setFistNameInput('fistName'), authorUpdatePage.setLastNameInput('lastName')]);

    expect(await authorUpdatePage.getFistNameInput()).to.eq('fistName', 'Expected FistName value to be equals to fistName');
